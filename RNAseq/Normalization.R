...
#Load packages
pack_R <- c("biomaRt","tidyverse","readxl","assertthat","edgeR")
...
par(mfrow=c(1,2))
plot(density(log2(count_cd4[,1]+1)))
...
#density before removing
par(mfrow=c(1,2))
plot(density(log2(count_cd8[,1]+1)))
...
count_cd4_tmm <- cpm(count_cd4_tmm, log=TRUE)
...
count_cd8_tmm <- cpm(count_cd8_tmm, log=TRUE)
