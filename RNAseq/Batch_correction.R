...
pdf(paste0(figure_folder, "pca_before_norm.pdf"))
...
covar_mat <- data.frame(cbind(metadata$Disease, metadata$State, metadata$Sample_Type)) # variables to keep
colnames(covar_mat) <- c("Disease", "State", "Sample_Type")
...
pdf(paste0(figure_folder, "pca_after_norm.pdf"))
